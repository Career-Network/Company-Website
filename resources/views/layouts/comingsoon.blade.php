<section class="container-cs" style="padding-top: 100px">
    <div class="coming-soon-container" style="background: url({{  asset('assets/img/coming-soon/bg-coming-soon.png') }})">
        <img class="cs-frame" src='{{ asset('assets/img/coming-soon/frame-coming-soon.png') }}' alt="Coming soon frame" />
        <img class="cs-illustration" src='{{ asset('assets/img/coming-soon/illustration-coming-soon.png') }}' alt="Coming soon illustration" />
    </div>
    <div class="desc-container">
        <h1 class="heading">We’re Launching Soon!</h1>
        <p class="sub-heading">We are currently working hand on this page</p>
    </div>
</section>
